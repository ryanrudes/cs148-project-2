...
def stub_dataset_and_model(monkeypatch):
    ...
    * ``wandb`` calls are no‑ops to avoid network activity during tests.
    ...
    monkeypatch.setattr(training, "build_deit3", lambda size, num_classes=10, drop_path_rate=None, image_size=224, use_flash_attention=False: TinyModel(num_classes))
